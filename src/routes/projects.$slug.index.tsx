          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <Badge variant="outline" className="bg-success/10 text-success border-success/30 text-xs">
                <Activity className="h-3 w-3 mr-1" /> Local
              </Badge>
            </div>
            {project.description && (
                ...
                {project.scripts.length > 0 ? (
                  <div className="space-y-1.5">
                    {project.scripts.map((s) => (
                      <div key={s} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-zinc-900/50 border border-zinc-800/60 text-xs font-mono">
                        <span className="text-primary">$</span>
                        <span className="text-zinc-300">npm run {s}</span>
                      </div>
                    ))}
                  </div>
                ) : (
      ...
      <div className={`grid grid-cols-1 ${hasApk ? "md:grid-cols-3" : "md:grid-cols-2"} gap-6`}>
        ...
        <div className="border border-border/30 rounded-xl p-4 bg-zinc-950/40 flex flex-col justify-between h-32 relative overflow-hidden">
          ...
          <button className="w-full relative z-10 inline-flex items-center justify-center rounded-lg border border-zinc-700 bg-transparent px-3 py-2 text-[10px] font-bold text-zinc-300 hover:bg-zinc-800/80 active:scale-95 transition-all">
            Launch Virtual Device Simulator
          </button>
        </div>
        ...
        {hasApk && (
          <div className="border border-border/30 rounded-xl p-4 bg-zinc-950/40 flex flex-col justify-between h-32 relative overflow-hidden">
            ...
            <a
              href={project.apkUrl}
              download
              className="w-full relative z-10 inline-flex items-center justify-center rounded-lg bg-emerald-600 px-3 py-2 text-[10px] font-bold text-white hover:bg-emerald-500 active:scale-95 transition-all shadow-[0_4px_12px_rgba(16,185,129,0.2)] text-center cursor-pointer"
            >
